...
The criterion for finding confounders of `gender` is that after adding another variable into the existed model,the change of slope of gender is over 10%.

After running the code,I find that for variable `dept`,`clin`,``cert`,`prate` and `exper`
...
interaction = vector("list",length = 6)
for (i in 2:7) {
  fitml_inter = 
    as.formula(paste0("log(avg_salary) ~ gender * ",names(law_data_change)[i],collapse = ""))
...
for (i in 1:6) {
  interaction_p_value[[i]] = broom::tidy(interaction[[i]]) %>% 
    mutate(
      model1 = paste0("log(avg_salary) ~ gender *",names(law_data_change)[i + 1],collapse = "")
    )
}
...
As we can see from the result, when we focus on p-value,the only interaction is `rank`. The effect of gender on salary is different with respect to the rank. 

```{r}
reg_y_prate = lm(log(avg_salary)~ gender * rank + dept + clin + cert + exper + prate, data = law_data_change)
# prate is not significant 
# Holding all other variables constant, prate does not have significant assocation with average salary. We delete prate
## Question:or we just keep prate because we want to see the association

# final model
reg_n_prate = lm(log(avg_salary)~ gender * rank + dept + clin + cert + exper, data = law_data_change) 
summary(reg_n_prate)
```
...
```{r}
law_model = lm(log(avg_salary)~ gender * rank + dept + clin + cert + exper, data = law_data_change)
