...
Also,the change in slope of gender when adding `rank` is very close to 10% and according to previous research,we finally add `rank` as a confounder.
...
* After refering some papers, we found " "" ", may be effect measure modifiers of the association between gender and salary. Then we use anova to test the interaction.
...
# interaction clin*gender
reg_in_clin = lm(log(avg_salary)~ gender * clin + rank + cert + dept + exper, data = law_data_change)
anova(reg_in_clin)
...
exper_6_9_minus =
  law_trans_minus %>% 
  filter(exper > 6 & exper <= 9)

exper_9_14_minus = 
  law_trans_minus %>% 
  filter(exper > 9 & exper <= 14)
...
final_law_model = lm(log(avg_salary)~ gender * rank + dept + clin + cert + exper * gender, data = law_trans_minus)
