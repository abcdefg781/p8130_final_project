...
confounder = p_value %>% 
    ...
    mutate(change_rate = round((estimate + 0.3853044)/(-0.3853044),3)) %>% 
...
>>>>>>> a39ffc89d97ad995e0237304a4ce25a4f67520ba
}
...
# Rank = Assistant
fit_assi = lm(log(avg_salary)~ gender + dept + clin + cert + prate + exper, data = rank_assi)
summary(fit_assi)
step(fit_assi)
lm(log(avg_salary)~ gender + dept + cert + prate + exper, data = rank_assi) %>% 
  summary()

# Rank = Associate: gender not significant
fit_asso = lm(log(avg_salary)~ gender + dept + clin + cert + prate + exper, data = rank_asso)
summary(fit_asso)
step(fit_asso)

# Rank = Full professor: gender not significant
fit_prof = lm(log(avg_salary)~ gender + dept + clin + cert + prate + exper, data = rank_prof)
summary(fit_prof)
step(fit_prof)
```
