...
for (i in 2:6) {
  fitml_inter = 
    as.formula(paste0("log(avg_salary) ~ gender+dept+clin+cert+exper+gender * ",names(law_data_change)[i],collapse = ""))
...
for (i in 1:5) {
  interaction_p_value[[i]] = broom::tidy(interaction[[i]]) %>% 
    mutate(
      model1 = paste0("log(avg_salary) ~ gender+dept+clin+cert+exper+gender *",names(law_data_change)[i + 1],collapse = "")
    )
}
...
As we can see from the result, when we focus on p-value,the only interaction is `rank` , `clin` and `exper`. The effect of gender on salary is different with respect to the rank and exper. 

```{r}
# final model
reg_n_prate = lm(log(avg_salary)~ gender*rank + gender*exper+ gender*clin + dept + cert , data = law_data_change) 
summary(reg_n_prate)
```
...
# Rank = Assistant
fit_assi = lm(log(avg_salary)~ gender*exper + gender*clin + dept+ cert, data = rank_assi)
summary(fit_assi)

# Rank = Associate: gender not significant
# means no discrimination between male and female in rank of associate ?
fit_asso = lm(log(avg_salary)~ gender*exper + gender*clin + dept+ cert, data = rank_asso)
summary(fit_asso)

# Rank = Full professor: gender not significant
# means no discrimination between male and female in rank of full professor?
fit_prof =lm(log(avg_salary)~  gender*exper + gender*clin + dept+ cert, data = rank_prof)
summary(fit_prof)
```
```{r}
# seperate regression models for clin
clin_data = 
  law_data_change %>% 
  filter(clin == "Primarily clinical emphasis") 

research_data =
  law_data_change %>% 
  filter(clin == "Primarily research emphasis") 


# clin = Primarily clinical emphasis
fit_clin = lm(log(avg_salary)~ gender*exper + gender*rank + dept+ cert, data = clin_data)
summary(fit_clin)
# clin = Primarily research emphasis
fit_research = lm(log(avg_salary)~ gender*exper + gender*rank + dept+ cert, data = research_data)
summary(fit_research)
...
```{r}
law_model = lm(log(avg_salary)~ gender * rank + dept + gender*clin + cert + exper * gender, data = law_data_change)
...
# Rank = Assistant
fit_assi_minus = lm(log(avg_salary)~ gender*exper + gender*clin + dept+ cert, data = rank_assi_minus)
summary(fit_assi_minus)

# Rank = Associate: gender not significant
# means no discrimination between male and female in rank of associate ?
fit_asso_minus = lm(log(avg_salary)~ gender*exper + gender*clin + dept+ cert, data = rank_asso_minus)
summary(fit_asso_minus)

# Rank = Full professor: gender not significant
# means no discrimination between male and female in rank of full professor?
fit_prof_minus =lm(log(avg_salary)~  gender*exper + gender*clin + dept+ cert, data = rank_prof_minus)
summary(fit_prof_minus)
```
```{r}
# seperate regression models for clin after delete the outlier
clin_data_minus = 
  law_trans_minus %>% 
  filter(clin == "Primarily clinical emphasis") 
...
fit_clin_minus = lm(log(avg_salary)~ gender*exper + gender*rank + dept+ cert, data = clin_data_minus)
summary(fit_clin_minus)

# clin = Primarily research emphasis
fit_research_minus = lm(log(avg_salary)~ gender*exper + gender*rank + dept+ cert, data = research_data_minus)
summary(fit_research_minus)
```
```{r}
#final model after deleting the outlier
final_law_model = lm(log(avg_salary)~ gender * rank + dept + gender*clin + cert + exper * gender, data = law_trans_minus)
